...
from mil_models import find_mil_model as initiate_model
from mil_models.model_clam import CLAM_MB, CLAM_SB, Attn_Net
...
os.environ['UNI_CKPT_PATH'] = '/storage/Pathology/codes/EasyMIL/models/ckpts/uni.bin'
os.environ['CONCH_CKPT_PATH'] = '/storage/Pathology/codes/EasyMIL/models/ckpts/conch.pth'
os.environ['CTRANSPATH_CKPT_PATH'] = '/storage/Pathology/codes/EasyMIL/models/ckpts/ctranspath.pth'
os.environ['DISFM_CKPT_PATH'] = '/storage/Pathology/codes/EasyMIL/models/ckpts/distill_87499.pth'
...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
    ...
    with torch.inference_mode():
        if isinstance(model, (CLAM_SB, CLAM_MB)):
            ...
            model_results_dict = model(features)
            logits = model_results_dict['wsi_logits']
            Y_prob = model_results_dict['wsi_prob']
            Y_hat = int(model_results_dict['wsi_label'])
            A = model_results_dict['attention']
...
if __name__ == '__main__':
    ...
    def_seg_params = {'seg_level': -1, 'sthresh': 1, 'mthresh': 7, 'close': 2, 'use_otsu': False, 
    ...
    model_kwargs = {"model_name": model_args.model_name, "in_dim": model_args.in_dim, "n_classes": model_args.n_classes, "drop_out": model_args.drop_out, "task_type": model_args.task_type}
    
    model = initiate_model(**model_kwargs)
    
    try:
        msg = model.load_state_dict(torch.load(model_args.ckpt_path)['state_dict'])
        print(msg)
    except:
        msg = model.load_state_dict(torch.load(model_args.ckpt_path))
        print(msg)

    model = model.to(device)

    feature_extractor, img_transforms = get_encoder(encoder_args.model_name, device, target_img_size=encoder_args.target_img_size)
    if encoder_args.model_name in ['resnet50_trunc', 'uni_v1', 'conch_v1']: # plip already loaded 
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        if data_args.process_list is not None:
            process_stack.to_csv('heatmaps/results/{}.csv'.format(data_args.process_list.split('/')[-1].replace('.csv', '')), index=False)
        else:
